...
**Decision: PostgreSQL.** MongoDB's flexibility is not needed; its integrity guarantees are weaker exactly where ReyDesk is most sensitive (audit, permissions, SLA).
...
- **VPS 1 — ReyDesk application server:** Nginx (HTTPS termination, reverse proxy, frontend delivery, API routing, WebSocket upgrades, security headers, request-size limits, basic rate limiting), the ReyDesk frontend, the Node.js API, authentication, the WebSocket signalling service, background workers, Redis, PostgreSQL, PgBouncer where beneficial, lightweight Prometheus exporters, and application logging agents. Recommended: **8 vCPU / 16 GB RAM minimum, fast NVMe, reliable datacentre networking, automated backups**. 32 GB may be used where pricing is attractive but is not required initially.
- **VPS 2 and VPS 3 — relay/TURN nodes:** coturn + the ReyDesk relay daemon + relay health endpoints + metrics exporter + structured logging + OpenTelemetry integration. Two nodes for resilience.
...
- **VPS 2 — TURN + relay node 1:** coturn, ReyDesk WebSocket/session relay, relay health endpoints, metrics exporter, structured logging, OpenTelemetry.
...
- Credentials are **short-lived and generated by the ReyDesk backend** — never static credentials embedded in clients. Provide ICE server information dynamically, e.g.:

```json
{
  "iceServers": [
    { "urls": ["stun:turn1.reydesk.com"] },
    {
      "urls": [
        "turn:turn1.reydesk.com:3478?transport=udp",
        "turn:turn2.reydesk.com:3478?transport=udp",
        "turns:turn1.reydesk.com:443?transport=tcp"
      ],
...
### 5.15 Go-live exit gate

Before a public production launch, the following release-hardening items must be complete and evidenced in staging: agent update apply with rollback health checks; signed Windows MSI/helper artifacts with SmartScreen-friendly distribution; two production coturn/relay nodes and a NAT/media test lab; S3-compatible object storage for recordings and large attachments; a durable webhook retry/replay-protection queue; repeatable k6/Playwright load tests at the 500 WebSocket/50-session target and headroom target; native Android/iOS packaging with push and signing; and on-device execution for approved scripts. The detailed checklist and execution order live in `docs/12-roadmap.md` under **Go-live stage — remaining release gates**.
...
**Separated now.** The remote-control network layer — two relay/TURN nodes (coturn + ReyDesk relay) — is separated first because active remote sessions are the most sensitive to relay failure and media is the most bandwidth-sensitive component.
