...
interface WorkspaceSettings {
  ...
  remote_support: {
    ...
    file_transfer_limit_mb: number
    require_elevated_action_reconsent: boolean
    unattended_access_policy: 'disabled' | 'approved_devices' | 'all_enrolled_devices'
  }
  endpoints: {
    ...
    personal_device_policy: 'allow_support_only' | 'block_unenrolled' | 'allow_all'
    inventory_collection: { hardware: boolean; software: boolean; processes: boolean; services: boolean; network: boolean }
    retire_after_offline_days: number
  }
  monitoring: {
    ...
    alert_routing: 'on_call_team' | 'service_desk' | 'managers' | 'custom'
    deduplication_window_minutes: number
    recovery_notifications: boolean
    escalation_delay_minutes: number
    notification_channels: Array<'in_app' | 'email' | 'push' | 'webhook'>
  }
  integrations: {
    webhook_events: string[]
    webhook_retry_attempts: number
    webhook_secret_rotation_days: number
    connection_test_timeout_seconds: number
    oauth_expiry_warning_days: number
  }
  data_retention: {
    audit_days: number
    ...
    purge_schedule: 'manual' | 'daily' | 'weekly'
  }
  assets: {
    warranty_expiry_emails: boolean
  }
}
...
const DEFAULT_SETTINGS: WorkspaceSettings = {
  ...
  monitoring: { create_tickets_by_default: true, offline_ticket_mode: 'alert_only', default_ticket_priority: 'p3', default_severity: 'warning', maintenance_windows: [], alert_routing: 'on_call_team', deduplication_window_minutes: 30, recovery_notifications: true, escalation_delay_minutes: 30, notification_channels: ['in_app'] },
  integrations: { webhook_events: ['*'], webhook_retry_attempts: 3, webhook_secret_rotation_days: 90, connection_test_timeout_seconds: 10, oauth_expiry_warning_days: 14 },
  data_retention: { audit_days: 365, recording_days: 30, notification_days: 90, ticket_days: 0, attachment_days: 0, chat_days: 90, telemetry_days: 30, legal_hold_enabled: false, purge_schedule: 'daily' },
  assets: { warranty_expiry_emails: true },
}
...
function normalizeWorkspaceSettings(raw: unknown): WorkspaceSettings {
  ...
  return {
    ...
    assets: { ...DEFAULT_SETTINGS.assets, ...(source.assets ?? {}) },
  }
}
...
function OperationalSettings({ tab }: { tab: 'portal' | 'remote' | 'devices' | 'monitoring' | 'data' }) {
  ...
  if (tab === 'remote') return <SettingSection title="Remote support" description="Set safe defaults for attended and unattended support sessions. Endpoint consent remains mandatory for attended access."><>{error && <Alert kind="error">{error}</Alert>}{message && <Alert kind="info">{message}</Alert>}<ToggleRow label="Require endpoint consent" description="Never allow attended screen sharing to start without explicit user approval." checked={settings.remote_support.require_consent} onChange={(v) => void update({ remote_support: { ...settings.remote_support, require_consent: v } })} /><div className="settings-form-grid"><Field label="Default session expiry (minutes)"><input className="field-input" type="number" min={5} max={1440} value={settings.remote_support.default_expiry_minutes} onChange={(e) => void update({ remote_support: { ...settings.remote_support, default_expiry_minutes: Number(e.target.value) } })} /></Field><Field label="Default recording mode"><select className="field-input" value={settings.remote_support.default_recording_mode} onChange={(e) => void update({ remote_support: { ...settings.remote_support, default_recording_mode: e.target.value as WorkspaceSettings['remote_support']['default_recording_mode'] } })}><option value="off">Off</option><option value="metadata">Metadata only</option><option value="video">Video</option></select></Field><Field label="Recording retention (days)"><input className="field-input" type="number" min={1} max={3650} value={settings.remote_support.recording_retention_days} onChange={(e) => void update({ remote_support: { ...settings.remote_support, recording_retention_days: Number(e.target.value) } })} /></Field><Field label="Maximum session duration (minutes)" hint="Hard upper bound for attended and unattended sessions."><input className="field-input" type="number" min={5} max={10080} value={settings.remote_support.max_session_duration_minutes} onChange={(e) => void update({ remote_support: { ...settings.remote_support, max_session_duration_minutes: Number(e.target.value) } })} /></Field><Field label="Inactivity timeout (minutes)" hint="Use 0 to keep sessions open until ended manually."><input className="field-input" type="number" min={0} max={1440} value={settings.remote_support.inactivity_timeout_minutes} onChange={(e) => void update({ remote_support: { ...settings.remote_support, inactivity_timeout_minutes: Number(e.target.value) } })} /></Field><Field label="File-transfer limit (MB)"><input className="field-input" type="number" min={1} max={2048} value={settings.remote_support.file_transfer_limit_mb} onChange={(e) => void update({ remote_support: { ...settings.remote_support, file_transfer_limit_mb: Number(e.target.value) } })} /></Field><Field label="Unattended access policy"><select className="field-input" value={settings.remote_support.unattended_access_policy} onChange={(e) => void update({ remote_support: { ...settings.remote_support, unattended_access_policy: e.target.value as WorkspaceSettings['remote_support']['unattended_access_policy'] } })}><option value="disabled">Disabled</option><option value="approved_devices">Approved devices only</option><option value="all_enrolled_devices">All enrolled devices</option></select></Field></div><ToggleRow label="Require re-consent for elevated actions" description="Ask for fresh approval before terminal, process, service, or other elevated actions." checked={settings.remote_support.require_elevated_action_reconsent} onChange={(v) => void update({ remote_support: { ...settings.remote_support, require_elevated_action_reconsent: v } })} /><ToggleRow label="Allow file transfer by default" description="New sessions may request file transfer; technicians can still scope each session." checked={settings.remote_support.allow_file_transfer} onChange={(v) => void update({ remote_support: { ...settings.remote_support, allow_file_transfer: v } })} /><ToggleRow label="Allow clipboard by default" description="Enable clipboard synchronization in newly generated support sessions." checked={settings.remote_support.allow_clipboard} onChange={(v) => void update({ remote_support: { ...settings.remote_support, allow_clipboard: v } })} /><ToggleRow label="Allow elevated terminal by default" description="Keep disabled unless your support policy explicitly permits terminal access." checked={settings.remote_support.allow_terminal} onChange={(v) => void update({ remote_support: { ...settings.remote_support, allow_terminal: v } })} /><ToggleRow label="Allow process and service management" description="Controls the default request; endpoint consent and technician permissions still apply." checked={settings.remote_support.allow_system_manage} onChange={(v) => void update({ remote_support: { ...settings.remote_support, allow_system_manage: v } })} /></></SettingSection>
  if (tab === 'devices') return <SettingSection title="Devices & agents" description="Choose how enrolled endpoints report health and how new devices join the organization."><>{error && <Alert kind="error">{error}</Alert>}{message && <Alert kind="info">{message}</Alert>}<div className="settings-form-grid"><Field label="Offline after (minutes)" hint="Used to classify a device as offline."><input className="field-input" type="number" min={1} max={1440} value={settings.endpoints.offline_after_minutes} onChange={(e) => void update({ endpoints: { ...settings.endpoints, offline_after_minutes: Number(e.target.value) } })} /></Field><Field label="Heartbeat interval (seconds)"><input className="field-input" type="number" min={10} max={3600} value={settings.endpoints.heartbeat_interval_seconds} onChange={(e) => void update({ endpoints: { ...settings.endpoints, heartbeat_interval_seconds: Number(e.target.value) } })} /></Field><Field label="Enrollment code expiry (minutes)"><input className="field-input" type="number" min={5} max={1440} value={settings.endpoints.enrollment_code_expiry_minutes} onChange={(e) => void update({ endpoints: { ...settings.endpoints, enrollment_code_expiry_minutes: Number(e.target.value) } })} /></Field><Field label="Minimum agent version" hint="Leave blank to allow any installed version."><input className="field-input mono" value={settings.endpoints.minimum_agent_version} onChange={(e) => void update({ endpoints: { ...settings.endpoints, minimum_agent_version: e.target.value } })} placeholder="1.4.0" /></Field><Field label="Retire after offline (days)" hint="Use 0 to disable automatic retirement."><input className="field-input" type="number" min={0} max={3650} value={settings.endpoints.retire_after_offline_days} onChange={(e) => void update({ endpoints: { ...settings.endpoints, retire_after_offline_days: Number(e.target.value) } })} /></Field><Field label="Personal-device policy"><select className="field-input" value={settings.endpoints.personal_device_policy} onChange={(e) => void update({ endpoints: { ...settings.endpoints, personal_device_policy: e.target.value as WorkspaceSettings['endpoints']['personal_device_policy'] } })}><option value="allow_support_only">Allow support sessions only</option><option value="block_unenrolled">Block unenrolled personal devices</option><option value="allow_all">Allow all device types</option></select></Field></div><ToggleRow label="Require enrollment approval" description="Keep newly enrolled devices pending until an administrator approves them." checked={settings.endpoints.enrollment_approval_required} onChange={(v) => void update({ endpoints: { ...settings.endpoints, enrollment_approval_required: v } })} /><ToggleRow label="Automatic agent updates" description="Allow enrolled agents to receive approved updates from the stable channel." checked={settings.endpoints.automatic_updates} onChange={(v) => void update({ endpoints: { ...settings.endpoints, automatic_updates: v } })} /><div className="settings-subsection"><h3>Inventory collection</h3><p>Choose which endpoint facts are collected for managed-device views and monitoring.</p><ToggleRow label="Hardware inventory" description="CPU, memory, operating system, and device identity." checked={settings.endpoints.inventory_collection.hardware} onChange={(v) => void update({ endpoints: { ...settings.endpoints, inventory_collection: { ...settings.endpoints.inventory_collection, hardware: v } } })} /><ToggleRow label="Software inventory" description="Installed applications and versions." checked={settings.endpoints.inventory_collection.software} onChange={(v) => void update({ endpoints: { ...settings.endpoints, inventory_collection: { ...settings.endpoints.inventory_collection, software: v } } })} /><ToggleRow label="Processes and services" description="Collect process and service state data from agents." checked={settings.endpoints.inventory_collection.processes || settings.endpoints.inventory_collection.services} onChange={(v) => void update({ endpoints: { ...settings.endpoints, inventory_collection: { ...settings.endpoints.inventory_collection, processes: v, services: v } } })} /><ToggleRow label="Network inventory" description="Collect network adapter and connectivity facts." checked={settings.endpoints.inventory_collection.network} onChange={(v) => void update({ endpoints: { ...settings.endpoints, inventory_collection: { ...settings.endpoints.inventory_collection, network: v } } })} /></div><ToggleRow label="Allow self-enrollment" description="Permit technicians to generate one-time enrollment codes from the Devices page." checked={settings.endpoints.allow_self_enrollment} onChange={(v) => void update({ endpoints: { ...settings.endpoints, allow_self_enrollment: v } })} /></></SettingSection>
  if (tab === 'monitoring') return <SettingSection title="Monitoring defaults" description="Set the default response when endpoint metrics generate alerts. Individual rules can override these values."><>{error && <Alert kind="error">{error}</Alert>}{message && <Alert kind="info">{message}</Alert>}<ToggleRow label="Create tickets by default" description="New monitoring rules start with automatic ticket creation enabled." checked={settings.monitoring.create_tickets_by_default} onChange={(v) => void update({ monitoring: { ...settings.monitoring, create_tickets_by_default: v } })} /><Field label="Offline device response" hint="Laptops that are shut down can raise alerts without opening tickets."><select className="field-input" value={settings.monitoring.offline_ticket_mode} onChange={(e) => void update({ monitoring: { ...settings.monitoring, offline_ticket_mode: e.target.value as WorkspaceSettings['monitoring']['offline_ticket_mode'] } })}><option value="alert_only">Alert only — recommended for laptops</option><option value="ticket">Open a ticket</option></select></Field><div className="settings-form-grid"><Field label="Default ticket priority"><select className="field-input" value={settings.monitoring.default_ticket_priority} onChange={(e) => void update({ monitoring: { ...settings.monitoring, default_ticket_priority: e.target.value } })}><option value="p1">P1 — Critical</option><option value="p2">P2 — High</option><option value="p3">P3 — Normal</option><option value="p4">P4 — Low</option></select></Field><Field label="Default severity"><select className="field-input" value={settings.monitoring.default_severity} onChange={(e) => void update({ monitoring: { ...settings.monitoring, default_severity: e.target.value as WorkspaceSettings['monitoring']['default_severity'] } })}><option value="info">Info</option><option value="warning">Warning</option><option value="critical">Critical</option></select></Field><Field label="Alert routing"><select className="field-input" value={settings.monitoring.alert_routing} onChange={(e) => void update({ monitoring: { ...settings.monitoring, alert_routing: e.target.value as WorkspaceSettings['monitoring']['alert_routing'] } })}><option value="on_call_team">On-call team</option><option value="service_desk">Service desk</option><option value="managers">Managers</option><option value="custom">Custom policy</option></select></Field><Field label="Deduplication window (minutes)"><input className="field-input" type="number" min={0} max={10080} value={settings.monitoring.deduplication_window_minutes} onChange={(e) => void update({ monitoring: { ...settings.monitoring, deduplication_window_minutes: Number(e.target.value) } })} /></Field><Field label="Escalation delay (minutes)"><input className="field-input" type="number" min={0} max={10080} value={settings.monitoring.escalation_delay_minutes} onChange={(e) => void update({ monitoring: { ...settings.monitoring, escalation_delay_minutes: Number(e.target.value) } })} /></Field></div><ToggleRow label="Send recovery notifications" description="Notify the selected channels when a device or monitoring rule returns to a healthy state." checked={settings.monitoring.recovery_notifications} onChange={(v) => void update({ monitoring: { ...settings.monitoring, recovery_notifications: v } })} /><div className="settings-subsection"><h3>Notification channels</h3><p>Choose where monitoring alerts and recoveries are routed by default.</p><div className="settings-channel-grid"><label><input type="checkbox" checked={settings.monitoring.notification_channels.includes('in_app')} onChange={(e) => void update({ monitoring: { ...settings.monitoring, notification_channels: e.target.checked ? [...settings.monitoring.notification_channels, 'in_app'] : settings.monitoring.notification_channels.filter((channel) => channel !== 'in_app') } })} />In-app</label><label><input type="checkbox" checked={settings.monitoring.notification_channels.includes('email')} onChange={(e) => void update({ monitoring: { ...settings.monitoring, notification_channels: e.target.checked ? [...settings.monitoring.notification_channels, 'email'] : settings.monitoring.notification_channels.filter((channel) => channel !== 'email') } })} />Email</label><label><input type="checkbox" checked={settings.monitoring.notification_channels.includes('push')} onChange={(e) => void update({ monitoring: { ...settings.monitoring, notification_channels: e.target.checked ? [...settings.monitoring.notification_channels, 'push'] : settings.monitoring.notification_channels.filter((channel) => channel !== 'push') } })} />Push</label><label><input type="checkbox" checked={settings.monitoring.notification_channels.includes('webhook')} onChange={(e) => void update({ monitoring: { ...settings.monitoring, notification_channels: e.target.checked ? [...settings.monitoring.notification_channels, 'webhook'] : settings.monitoring.notification_channels.filter((channel) => channel !== 'webhook') } })} />Webhook</label></div></div><div className="settings-subsection"><h3>Maintenance windows</h3><p>Maintenance schedules are stored here as policy metadata; monitoring rules can still define their own windows.</p><textarea className="field-input mono" rows={3} value={JSON.stringify(settings.monitoring.maintenance_windows, null, 2)} onChange={(e) => { try { const value = JSON.parse(e.target.value) as Array<{ start: string; end: string }>; void update({ monitoring: { ...settings.monitoring, maintenance_windows: Array.isArray(value) ? value : [] } }) } catch { /* wait for valid JSON */ } }} aria-label="Maintenance windows JSON" placeholder={'[{"start":"2026-01-01T22:00:00Z","end":"2026-01-02T02:00:00Z"}]'} /></div><p className="settings-note"><Icon name="alert" size={14} /> Individual monitoring rules always take precedence over these defaults.</p></></SettingSection>
  return <SettingSection title="Data retention" description="Retention policy guidance for tenant-scoped operational data. Actual purge jobs must be enabled in production infrastructure."><>{error && <Alert kind="error">{error}</Alert>}{message && <Alert kind="info">{message}</Alert>}<div className="settings-form-grid"><Field label="Audit logs (days)"><input className="field-input" type="number" min={30} max={3650} value={settings.data_retention.audit_days} onChange={(e) => void update({ data_retention: { ...settings.data_retention, audit_days: Number(e.target.value) } })} /></Field><Field label="Session recordings (days)"><input className="field-input" type="number" min={1} max={3650} value={settings.data_retention.recording_days} onChange={(e) => void update({ data_retention: { ...settings.data_retention, recording_days: Number(e.target.value) } })} /></Field><Field label="Notifications (days)"><input className="field-input" type="number" min={7} max={3650} value={settings.data_retention.notification_days} onChange={(e) => void update({ data_retention: { ...settings.data_retention, notification_days: Number(e.target.value) } })} /></Field><Field label="Tickets (days)" hint="Use 0 to retain indefinitely."><input className="field-input" type="number" min={0} max={3650} value={settings.data_retention.ticket_days} onChange={(e) => void update({ data_retention: { ...settings.data_retention, ticket_days: Number(e.target.value) } })} /></Field><Field label="Attachments (days)" hint="Use 0 to retain indefinitely."><input className="field-input" type="number" min={0} max={3650} value={settings.data_retention.attachment_days} onChange={(e) => void update({ data_retention: { ...settings.data_retention, attachment_days: Number(e.target.value) } })} /></Field><Field label="Chat (days)" hint="Use 0 to retain indefinitely."><input className="field-input" type="number" min={0} max={3650} value={settings.data_retention.chat_days} onChange={(e) => void update({ data_retention: { ...settings.data_retention, chat_days: Number(e.target.value) } })} /></Field><Field label="Telemetry (days)" hint="Use 0 to retain indefinitely."><input className="field-input" type="number" min={0} max={3650} value={settings.data_retention.telemetry_days} onChange={(e) => void update({ data_retention: { ...settings.data_retention, telemetry_days: Number(e.target.value) } })} /></Field><Field label="Purge schedule"><select className="field-input" value={settings.data_retention.purge_schedule} onChange={(e) => void update({ data_retention: { ...settings.data_retention, purge_schedule: e.target.value as WorkspaceSettings['data_retention']['purge_schedule'] } })}><option value="manual">Manual</option><option value="daily">Daily</option><option value="weekly">Weekly</option></select></Field></div><ToggleRow label="Legal hold enabled" description="Pause automated retention actions for this tenant while legal or compliance review is active." checked={settings.data_retention.legal_hold_enabled} onChange={(v) => void update({ data_retention: { ...settings.data_retention, legal_hold_enabled: v } })} /><ToggleRow label="Asset expiry emails" description="Email asset owners when a warranty or licence enters its 30-day expiry window. Each due date notifies once." checked={settings.assets.warranty_expiry_emails} onChange={(v) => void update({ assets: { warranty_expiry_emails: v } })} /><p className="settings-note"><Icon name="clock" size={14} /> Changes are stored as policy metadata. Configure scheduled purge workers before relying on them for compliance.</p></></SettingSection>
}
...
function IntegrationsSettings() {
  const { settings, loading, error, setError, save } = useWorkspaceSettings()
  const [message, setMessage] = useState<string | null>(null)
  if (loading || !settings) return <div className="settings-card"><span className="etch">Loading integration settings…</span></div>
  const update = async (patch: Partial<WorkspaceSettings>) => {
    try { await save(patch); setMessage('Integration settings saved.') }
    catch (err) { setError(err instanceof Error ? err.message : 'Could not save integration settings') }
  }
  return <div className="settings-card-grid">
    <SettingSection title="Webhook delivery" description="Set tenant-wide delivery defaults. Endpoint URLs, secrets, tests, and event subscriptions remain managed in Integrations.">
      <>{error && <Alert kind="error">{error}</Alert>}{message && <Alert kind="info">{message}</Alert>}
        <div className="settings-form-grid"><Field label="Retry attempts"><input className="field-input" type="number" min={1} max={10} value={settings.integrations.webhook_retry_attempts} onChange={(e) => void update({ integrations: { ...settings.integrations, webhook_retry_attempts: Number(e.target.value) } })} /></Field><Field label="Secret rotation reminder (days)"><input className="field-input" type="number" min={1} max={3650} value={settings.integrations.webhook_secret_rotation_days} onChange={(e) => void update({ integrations: { ...settings.integrations, webhook_secret_rotation_days: Number(e.target.value) } })} /></Field><Field label="Connection test timeout (seconds)"><input className="field-input" type="number" min={1} max={120} value={settings.integrations.connection_test_timeout_seconds} onChange={(e) => void update({ integrations: { ...settings.integrations, connection_test_timeout_seconds: Number(e.target.value) } })} /></Field></div>
        <Field label="Subscribed event patterns" hint="One pattern per line. Use * for all events or prefixes such as ticket.*."><textarea className="field-input mono" rows={4} value={settings.integrations.webhook_events.join('\n')} onChange={(e) => void update({ integrations: { ...settings.integrations, webhook_events: e.target.value.split(/\r?\n/).map((value) => value.trim()).filter(Boolean) } })} /></Field>
        <NavLink className="btn btn-primary btn-sm settings-card-link" to="/integrations"><Icon name="external" size={14} />Manage webhook endpoints</NavLink>
      </>
    </SettingSection>
    <SettingSection title="Connected services" description="Keep connector checks and OAuth expiry warnings visible to administrators."><><Field label="OAuth expiry warning (days)"><input className="field-input" type="number" min={1} max={365} value={settings.integrations.oauth_expiry_warning_days} onChange={(e) => void update({ integrations: { ...settings.integrations, oauth_expiry_warning_days: Number(e.target.value) } })} /></Field><p className="settings-note"><Icon name="shield" size={14} /> Connection tests should be run after credentials change. OAuth warnings are policy metadata until each connector exposes its expiry state.</p><NavLink className="btn btn-ghost btn-sm settings-card-link" to="/marketplace"><Icon name="external" size={14} />Open marketplace</NavLink></></SettingSection>
    <SettingSection title="Email and identity connectors" description="Email, Entra ID, and directory connectors are managed in their dedicated settings areas."><div className="settings-link-list"><NavLink to="/settings/email">Email channels <Icon name="forward" size={14} /></NavLink><NavLink to="/settings/active-directory">Directory sync <Icon name="forward" size={14} /></NavLink></div></SettingSection>
  </div>
}
